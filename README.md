...
modules that ship carry thirty-one tests of their own, and
...
`all`, `count_where`, `filtered_with`, `first`, `last`, `reversed`, `prepend`, `find`, `take`, `drop`, `concat`, `flatten`, `partition` and `zip`, none of them known to the
