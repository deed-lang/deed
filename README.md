...
modules that ship carry twenty-nine tests of their own, and
...
`all`, `count_where`, `filtered_with`, `first`, `last`, `reversed`, `prepend`, `find`, `take`, `drop`, `concat`, `flatten` and `partition`, none of them known to the
