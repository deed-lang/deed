...
modules that ship carry thirty-two tests of their own, and
...
`all`, `count_where`, `filtered_with`, `first`, `last`, `reversed`, `prepend`, `find`, `take`, `drop`, `concat`, `flatten`, `partition`, `zip` and `enumerate`, none of them known to the
