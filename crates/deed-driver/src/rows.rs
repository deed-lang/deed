...
//!
//! `DEED5001` names the effect, names the function and tells the reader to add
//! it to the `uses` clause. `DEED5002` names the entry that has to come out and
...
use deed_ast::{Item, Module};
use deed_diagnostics::{Applicability, Diagnostic, Span};
use deed_effects::{Effects, codes};
use deed_lexer::Trivia;
use deed_resolve::Resolutions;
...
/// there, and the same shape `deed fmt` prints otherwise. Written out rather
