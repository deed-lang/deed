...
//! return value was not, so `deed check` printed "so it becomes a runtime check"
...
use deed_interp::{Program, TestOutcome, codes, run_tests};
...
// and a Deed test that showed one could not pass. That meant the tier this file
// exists for was the one thing a Deed program could not test about itself, and
...
fn a_test_can_say_a_precondition_refuses_something() {
    ...
    // that is the statement being right rather than `DEED4025`.
    expect_pass(
        "module a\n\n\
         fn halve(n: Int) -> Int\n\
         \x20 where\n\
