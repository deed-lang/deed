...
//!
//! The decision this file is the consequence of is #90. Deed has one mutable
...
fn declaring_diverge_for_a_for_is_refused() {
    ...
    assert_eq!(
        ...
        vec![deed_effects::codes::UNUSED_EFFECT]
    );
}
...
// those in `examples/todo.deed` and they were the same three lines each time.
...
fn the_index_is_known_to_be_a_real_position() {
    ...
    let proven = checked
        .obligations
        .iter()
        .filter(|obligation| obligation.tier == deed_typeck::Tier::Proven)
        .count();
...
fn a_condition_with_nothing_to_be_about_is_refused() {
    ...
    assert_eq!(
        codes_of(&checked.diagnostics),
        vec![deed_typeck::codes::WHILE_WITHOUT_ACCUMULATOR]
    );
