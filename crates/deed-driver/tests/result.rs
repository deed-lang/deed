...
use deed_driver::{check_all, check_text};
...
fn check_with(src: &str, deps: &[&str]) -> (SourceMap, deed_driver::Checked) {
    let mut sources = SourceMap::new();
    let mut files = vec![sources.add("test.deed", src)];
    for (index, dep) in deps.iter().enumerate() {
        files.push(sources.add(format!("dep{index}.deed"), *dep));
    }
...
fn the_question_mark_needs_a_result() {
    ...
    assert_eq!(
        codes_of(&checked.diagnostics),
        vec![deed_typeck::codes::NOT_A_RESULT]
    );
...
fn the_question_mark_needs_somewhere_to_propagate_to() {
    ...
    assert!(
        codes_of(&checked.diagnostics).contains(&deed_typeck::codes::TRY_NEEDS_RESULT_RETURN),
        "{}",
        rendered(&sources, &checked.diagnostics)
    );
...
fn importing_a_name_the_language_provides_warns() {
    ...
    assert!(
        codes_of(&checked.diagnostics).contains(&deed_resolve::codes::SHADOWED_DECLARATION),
        "{}",
        rendered(&sources, &checked.diagnostics)
    );
...
fn result_has_no_methods() {
    // Found in examples/transfer.deed the moment `Result` stopped being unknown.
    let (sources, checked) = check(&with_errors(
    ...
    assert_eq!(
        codes_of(&checked.diagnostics),
        vec![deed_typeck::codes::NO_SUCH_FIELD]
    );
