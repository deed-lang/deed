...
fn forgetting_the_failure_case_is_an_error() {
    ...
    assert_eq!(
        ...
        vec![deed_typeck::codes::NON_EXHAUSTIVE_MATCH]
    );
...
fn a_wildcard_cannot_swallow_the_failure_case() {
    ...
    assert_eq!(
        codes_of(&checked.diagnostics),
        vec![deed_typeck::codes::CATCH_ALL_ON_CHOICE]
    );
...
fn ok_on_something_that_is_not_a_result_is_rejected() {
    ...
    assert!(
        codes_of(&checked.diagnostics).contains(&deed_typeck::codes::PATTERN_MISMATCH),
        "{}",
        rendered(&sources, &checked.diagnostics)
    );
...
fn a_binding_in_a_pattern_cannot_shadow_a_parameter() {
    // Found while writing examples/counter.deed, where the obvious name for the
    ...
    assert!(
        codes_of(&checked.diagnostics).contains(&deed_resolve::codes::SHADOWED_BINDING),
        "{}",
        rendered(&sources, &checked.diagnostics)
    );
}
