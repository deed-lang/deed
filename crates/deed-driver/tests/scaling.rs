...
fn check_time(source: &str) -> Duration {
    ...
    let checked = check_text(&mut sources, "bench.deed", source.to_string());
...
fn the_generated_source_is_actually_checked() {
    ...
    let mut sources = SourceMap::new();
    let checked = check_text(&mut sources, "bench.deed", generated(3));
    assert!(
...
fn every_pass_is_timed() {
    let mut sources = SourceMap::new();
    let checked = check_text(&mut sources, "bench.deed", generated(40));
