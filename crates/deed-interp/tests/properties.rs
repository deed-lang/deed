...
fn the_counter_example_has_a_property_and_it_passes() {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../examples/counter.deed");
    let source = std::fs::read_to_string(path).expect("examples/counter.deed should exist");

    let mut sources = SourceMap::new();
    let file = sources.add("examples/counter.deed", source);
    let lexed = tokenize(file, sources.file(file).text());
    let parsed = parse(file, &lexed.tokens);
    let resolved = resolve(file, &parsed.module, &Universe::new());
