...
pub fn row_sources(sig: &deed_ast::FnSig, contract: &deed_ast::Contract) -> Vec<usize> {
...
impl RowLowering {
    ...
    pub fn declaring(&mut self, variables: &[deed_ast::Ident]) {
    ...
    pub fn normalised(&self, uses: &[deed_ast::EffectRef]) -> Vec<RowEntry> {
...
///
/// There is no visibility modifier in Deed, so every item is exported. That is
