...
pub struct Diagnostic {
    /// Stable identifier, for example `VOW1002`. Codes are never reused.
    pub code: &'static str,
...
mod tests {
    ...
    fn primary_label_defaults_to_the_message() {
        ...
        let file = map.add("t.vow", "let x = 1");
        let d = Diagnostic::error("VOW0001", file, Span::new(0, 3), "something went wrong");
    ...
    fn builders_compose() {
        let mut map = SourceMap::new();
        let file = map.add("t.vow", "let x = 1");
        let d = Diagnostic::warning("VOW0002", file, Span::new(0, 3), "headline")
