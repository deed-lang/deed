...
mod tests {
    ...
    fn human_output_points_at_the_right_column() {
        ...
        let file = map.add("t.vow", source);
        let span = Span::new(
            source.find('x').unwrap() as u32,
            source.find('x').unwrap() as u32 + 1,
        );
        let d =
            Diagnostic::error("VOW9001", file, span, "example problem").with_primary_label("here");

        let text = render_human(&map, &d);
        assert!(text.starts_with("error[VOW9001]: example problem\n"));
        assert!(text.contains("--> t.vow:2:5"));
    ...
    fn underline_stops_at_the_end_of_the_line() {
        ...
        let mut map = SourceMap::new();
        let file = map.add("t.vow", source);
        let d = Diagnostic::error("VOW9002", file, Span::new(0, 8), "spans two lines");
    ...
    fn json_escapes_and_carries_locations() {
        ...
        let mut map = SourceMap::new();
        let file = map.add("t.vow", source);
        let d = Diagnostic::error("VOW9003", file, Span::new(8, 10), "unterminated \"string\"")
            .with_note("line one\nline two")
            .with_fix(
                "close it",
                Span::new(10, 10),
        ...
        assert!(json.contains("\"code\":\"VOW9003\""));
    ...
    fn json_fix_is_null_when_absent() {
        let mut map = SourceMap::new();
        let file = map.add("t.vow", "abc");
        let d = Diagnostic::error("VOW9004", file, Span::new(0, 1), "no fix");
