...
//!
//! ```text
//! cargo run -p vow-driver --example edit_loop --release
//! ```
//!
...
fn verify() {
    ...
    let ids: Vec<_> = texts
        .iter()
        .enumerate()
        .map(|(index, text)| sources.add(format!("m{index}.vow"), text.clone()))
        .collect();

    for checked in vow_driver::check_all(&sources, &ids) {
        if let Some(diagnostic) = checked.diagnostics.first() {
            panic!(
                "the generated workspace should be clean, and it is not:\n{}",
                vow_diagnostics::render_human(&sources, diagnostic)
            );
        }
    }
}
