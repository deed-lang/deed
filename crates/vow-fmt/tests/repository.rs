//! Every `.vow` file in the repository is in canonical form.
//!
...
#[test]
fn every_vow_file_in_the_repository_is_formatted() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let mut files = Vec::new();
    collect(&root, &mut files);
    files.sort();

    assert!(!files.is_empty(), "no `.vow` files found under {root:?}");
    ...
    assert!(
        ...
        "not in canonical form, run `vow fmt`:\n  {}",
        unformatted.join("\n  ")
    );
}
