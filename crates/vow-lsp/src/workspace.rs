...
//!
//! The unit of compilation in Vow is the set of files handed to the compiler,
...
//! says is the folder is the same answer `vow check src/` gives when a person
...
impl Workspace {
    ...
    /// Every `.vow` file under every folder, sorted.
    ///
...
fn collect(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            collect(&path, out);
        } else if path.extension().is_some_and(|ext| ext == "vow") {
            out.push(path);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn the_walk_finds_vow_files_and_skips_build_output() {
        ...
        std::fs::write(dir.join("src/a.vow"), "module a\n").unwrap();
        std::fs::write(dir.join("src/b.vow"), "module b\n").unwrap();
        std::fs::write(dir.join("src/notes.txt"), "not source").unwrap();
        std::fs::write(dir.join("target/old.vow"), "module old\n").unwrap();
        std::fs::write(dir.join(".git/hook.vow"), "module hook\n").unwrap();
        ...
        assert!(files[0].ends_with("a.vow"), "{files:?}");
        assert!(files[1].ends_with("b.vow"), "{files:?}");
